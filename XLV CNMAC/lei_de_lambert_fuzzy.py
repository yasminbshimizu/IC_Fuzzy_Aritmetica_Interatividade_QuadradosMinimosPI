...
def gdivisao_ntfuzzy(x_num, x_den):
    """    
...
def prev_mqmi_gfuzzy(y_data, solucoes):
    ...
    for s in range(len(solucoes)):
        ...
        for i in range(len(y_data)):
            ...
            soma = soma_iterativa_0(y_data[i][0], op_alpha2)
            x1, x2, defs = gdivisao_ntfuzzy(soma, alpha1)
...
def prev_mqmi_fuzzy_2Dplot(x_data, y_data, x_data_prev, y_data_prev, xlabel = r"$X_{fuzzy}$", ylabel = r"$Y_{fuzzy}$"):
    ...
    plt.title(f"Previsões de {xlabel} ($\div$)")
    #plt.legend(loc='lower left', bbox_to_anchor=(1, 0), borderaxespad=0.5)
    plt.legend()
    plt.xlabel(xlabel)
    plt.ylabel(ylabel)
    ...
    plt.savefig(f"Previsões de {xlabel} (Gráfico 2D de superfície)", bbox_inches='tight')
    plt.show()
...
def prev_mqmi_fuzzy_3Dplot(x_data, y_data, x_data_prev, y_data_prev, xlabel = r"$X_{fuzzy}$", ylabel = r"$Y_{fuzzy}$"):
    ...
    ax.set_title(f"Previsões de {xlabel} ($\div$)")
    #ax.legend(loc='lower left', bbox_to_anchor=(1, 0), borderaxespad=0.5)
    ax.legend()
    ax.set_xlabel(xlabel)
    ax.set_ylabel(ylabel)
    ...
    fig.savefig(f"Previsões de {xlabel}", bbox_inches='tight')
    
    
def prev_mqmi_gfuzzy_2Dplot(x_data, y_data, x_data_prev, y_data_prev, xlabel = r"$X_{fuzzy}$", ylabel = r"$Y_{fuzzy}$"):
    """
    Plota um conjunto de dados com x real e y fuzzy, e as previsões feitas pelo inverso do método de quadrados mínimos fuzzy com x fuzzy predito e y fuzzy conhecido. 
    ...
    cores = ["#5f2ad5", "deeppink", "indigo", "#5271ff"]   
    ...
    for s in range(len(x_data_prev)):
        plt.plot([], [], color=cores[s], alpha = 0.8, marker = ".", ms = 5, label=f"Previsão da S{s+1}")
        for i in range (len(x_data_prev[s])):
            y = [y_data_prev[i][0][1] for _ in range(len(x_data_prev[s][i]))]
            plt.plot(x_data_prev[s][i], y, color = cores[s], linewidth=1, alpha = 0.8)
            #plt.plot(x_data_prev[s][i], y, color = cores[s], linewidth=1, alpha = 0.8)
            x_pico = x_data_prev[s][i][-1]
            y_pico = y_data_prev[i][0][1]
    ...
    plt.title(f"Previsões de {xlabel} ($\div_g$)")
    plt.legend()
    plt.xlabel(xlabel)
    plt.ylabel(ylabel)
    plt.grid()
    
    plt.savefig(f"gPrevisões de {xlabel} (Gráfico 2D de superfície)", bbox_inches='tight')
    plt.show()
    
    
def prev_mqmi_gfuzzy_3Dplot(x_data, y_data, x_data_prev, y_data_prev, xlabel = r"$X_{fuzzy}$", ylabel = r"$Y_{fuzzy}$"):
    """
    Plota um conjunto de dados com x real e y fuzzy, e as previsões feitas pelo inverso do método de quadrados mínimos fuzzy com x fuzzy predito e y fuzzy conhecido - associados a função de pertinecia dos números fuzzy.
    ...
    for s in range(len(x_data_prev)):
        ...
        for i in range (len(x_data_prev[s])):
            y = [y_data_prev[i][0][1] for _ in range(len(x_data_prev[s][i]))]
            z_ = np.linspace(0,1, len(x_data_prev[s][i])//2)
            z = np.concatenate([z_,z_])
            ax.plot(x_data_prev[s][i], y, z, color = cores[s], linewidth=1, alpha = 0.8)
#            ax.plot(x_data_prev[s][i], y, z, color = cores[s], linewidth=1, alpha = 0.8)
            x_pico = x_data_prev[s][i][-1]
            y_pico = y_data_prev[i][0][1]
            ax.scatter(x_pico, y_pico, 1, color = cores[s], alpha = 0.8, s=5)
    ...
    ax.set_title(f"Previsões de {xlabel} ($\div_g$)")
    ax.legend()
    ax.set_xlabel(xlabel)
    ax.set_ylabel(ylabel)
    ax.set_zlabel('Pertinência')
    ax.view_init(elev=20, azim=75)
    fig.savefig(f"gPrevisões de {xlabel}", bbox_inches='tight')
...
def lei_lambert_gfuzzy(x_data, y_data, x_real = None, y_prev = None, titulo = "Ajuste por mínimos quadrados fuzzy", xlabel = "x", ylabel = "y"):
    ...
    Args:
        ...
        x_real (array-like, optional): valores reais originais de x, se diferentes dos usados em x_data (padrão: None).
        prevw (int, optional): número de pontos futuros a prever (padrão: 0).
        titulo (str, optional): título dos gráficos gerados (padrão: "Ajuste por mínimos quadrados fuzzy").
    ...
    if y_prev is not None:
        x_prev = prev_mqmi_gfuzzy(y_prev, solucoes)
        
        prev_mqmi_gfuzzy_2Dplot(x_ord, y_ord, x_prev, y_prev, xlabel = xlabel, ylabel = ylabel)
        prev_mqmi_gfuzzy_3Dplot(x_ord, y_ord, x_prev, y_prev, xlabel = xlabel, ylabel = ylabel)
        #plota_diametros_prev(x_prev, y_prev)
